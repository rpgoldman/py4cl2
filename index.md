...
- stderr and stdout of python process is read asynchronously from a separate thread - this does render the use of `with-output-to-string` useless for wrapping around python functions; a work-around is to set `py4cl2::*py4cl-tests*` to `t` and use `(uiop:process-info-output py4cl::*python*)`. (Refer [tests/tests.lisp](tests/test.lisp) for examples.)
